...
class DB {
    ...
    return this.db
      ...
      .then((doc) => {
        ...
        if (doc.exists && docData) {
          ...
          throw new Error(
            'No AUMT member exists for this acccount ' + fbUser.uid
          )
        }
      })
  }
  ...
  public setClubConfig = (config: ClubConfig): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection('config').doc('config').set(config)
  }
  ...
  public setClubSignupSem = (sem: 'S1' | 'S2' | 'SS'): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection('config').doc('config').update({
      clubSignupSem: sem,
    })
  }
  ...
  public submitCommitteeApplication = (
  ...
  public submitEvent = (eventData: AumtEvent): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection('events').doc(eventData.id).set(eventData)
  }
  ...
  public confirmMemberEventSignup = (
    eventId: string,
    uid: string,
    confirmed: boolean,
    waitlist?: boolean
  ...
  public removeTraining = (trainingId: string): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection(TRAINING_DB_PATH).doc(trainingId).delete()
  }
  ...
  public setMember = (
    ...
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection(MEMBER_DB_PATH).doc(memberId).set(memberData)
  }

  public addMultipleMembers = (
  ...
  public removeMultipleMembers = (memberIds: string[]): Promise<void> => {
    ...
    memberIds.forEach((id) => {
      const doc = this.db!.collection(MEMBER_DB_PATH).doc(id)
      batch.delete(doc)
    })
    return batch.commit()
  }
  ...
  public removeMember = (memberId: string): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
    return this.db.collection(MEMBER_DB_PATH).doc(memberId).delete()
  }
  ...
  public checkTrainingAttendanceExists = async (
    formId: string,
    sessionId: string
  ): Promise<boolean> => {
  ...
  public createTrainingAttendance = async (
    formId: string,
    sessionId: string
  ): Promise<void> => {
    if (!this.db) return Promise.reject('No db object')
  ...
  public getTrainingAttendance = async (
    formId: string,
    sessionId: string
  ): Promise<string[]> => {
  ...
  public setMemberTrainingAttendance = (
    formId: string,
    sessionId: string,
    memberId: string,
    attended: string[]
  ...
  public removeMemberFromForm = (
    ...
    return this.db.collection(TRAINING_DB_PATH).doc(formId).set(
      {
        sessions: sessionObj,
      },
      { merge: true }
  ...
  public listenToOneTraining = (
    ...
    this.listeners[listenerId] = this.db
      .collection(TRAINING_DB_PATH)
      .doc(formId)
