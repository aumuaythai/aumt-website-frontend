...
import { submitCommitteeApplication } from '../../../services/db'
...
export class CommitteeApplicationForm extends Component<
  CommitteeApplicationFormProps,
  CommitteeApplicationFormState
> {
  private rolesList = [
    'President',
    'Vice President',
  ...
  onSubmit = (vals: AumtCommitteeApp | any) => {
    ...
    submitCommitteeApplication(app)
      .then(() => {
        this.setState({ ...this.state, submitting: false, submitted: true })
      })
      .catch((e) => {
        notification.error({
          message: 'Error submitting committee application: ' + e.toString(),
        })
      ...
      return (
        <div className="committeeApplicationsContainer">
          <Result
            ...
            subTitle={
              'The current committee will contact you about an interview! Interviews will be held between Monday the 19th and Wednesday the 21st of October, in person at the university'
            }
          ></Result>
        </div>
      )
    ...
    return (
      <div className="committeeApplicationsFormContainer">
        ...
        <Form layout="vertical" onFinish={this.onSubmit} ref={this.formRef}>
          ...
          <Form.Item
            name="role1"
            label="Select your preferred role"
            rules={[{ required: true }]}
          >
            <Select
              value={this.state.currentRole1}
              style={{ width: 200 }}
              onChange={this.onRole1Select}
            >
              {this.rolesList.map((role, idx) => {
                return (
                  <Select.Option key={idx} value={role}>
                    {role}
                  </Select.Option>
                )
              })}
          ...
          <Form.Item
            name="whyrole1"
            label="Why have you decided this role?"
            rules={[{ required: true }]}
          >
            <Input.TextArea
              autoSize={{ maxRows: 6, minRows: 3 }}
              placeholder="Please explain what interests you about this role and being on the committee in general."
            ></Input.TextArea>
          </Form.Item>
          <Form.Item name="role2" label="Select a backup role (optional)">
            <Select
              allowClear
              value={this.state.currentRole2}
              style={{ width: 200 }}
              onChange={this.onRole2Select}
            >
          ...
          {this.state.currentRole2 ? (
            <Form.Item
              name="whyrole2"
              label="Why have you decided this role? (optional)"
            >
              <Input.TextArea
                autoSize={{ maxRows: 6, minRows: 3 }}
                placeholder="Please explain what interests you about this role and being on the committee in general."
