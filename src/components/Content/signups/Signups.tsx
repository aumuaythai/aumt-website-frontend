...
class Signups extends Component<SignupProps, SignupState> {
    ...
    db.getOpenForms()
      .then((forms) => {
        this.setState({
          ...this.state,
          loadingForms: false,
          dbListenerIds: forms.map((f) =>
            db.listenToOneTraining(f.trainingId, this.onDbChanges)
          ),
        })
      ...
      .catch((err) => {
        notification.error({
          message:
            'Error getting weekly trainings from db: ' + JSON.stringify(err),
        })
        this.setState({
          ...this.state,
          loadingForms: false,
  ...
  getDisplayName = (): string | null => {
    if (this.props.authedUser) {
      const displayName =
        this.props.authedUser.firstName +
        (this.props.authedUser.preferredName
          ? ` "${this.props.authedUser.preferredName}" `
          : ' ') +
  ...
  handleNewForms = (forms: AumtWeeklyTraining[]) => {
    this.setState({
      forms: forms,
      noFormText: forms.length
        ? ''
        : 'Weekly training slots will open on Sundays',
            ...
            if (!this.props.authedUserId) {
              ...
              return (
                <div key={form.trainingId} className="signupsNotPaidContainer">
                  <h2>{form.title}</h2>
                  <p>
                    Our records show you have not paid the membership fee for
                    this semester - once you do, you can sign up to trainings!
                  </p>
