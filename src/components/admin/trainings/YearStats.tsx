...
export class YearStats extends Component<YearStatsProps, YearStatsState> {
    ...
    if (this.props !== prevProps && this.props.forms) {
      ...
      const graphData = this.props.forms
        ...
        .map((form) => {
          ...
          const limit = Object.values(form.sessions).reduce(
            (sum, cur) => sum + cur.limit,
            0
          )
          return {
  ...
  customTooltip = (props: any) => {
    const currentForm = this.props.forms.find(
      (f) => f.trainingId === props.label
    )
    if (!currentForm) {
      return <div></div>
    }
    const tooltipValues = Object.values(currentForm.sessions).reduce(
      (vals, session) => {
    ...
    return (
      <div className="yearStatsTooltip">
        <div className="yearStatsTooltipTitle">{currentForm.title}</div>
        <div className="yearStatsTooltipRow">
          <div className="yearStatsTooltipName">TOTAL</div>
          <div className="yearStatsTooltipVal">
            {this.state.showPercent
              ? `${Math.round(
                  (tooltipValues['total'] * 100) / tooltipValues['limit']
                )}% (${tooltipValues['total']} / ${tooltipValues.limit})`
              : tooltipValues['total']}
          </div>
          <div className="clearBoth"></div>
        </div>
        {Object.values(currentForm.sessions)
          .sort((a, b) => a.position - b.position)
          .map((session) => {
            return (
              <div key={session.sessionId} className="yearStatsTooltipRow">
                <div className="yearStatsTooltipName">{session.title}</div>
                <div className="yearStatsTooltipVal">
                  {this.state.showPercent
                    ? `${tooltipValues[session.sessionId]} / ${session.limit}`
  ...
  render() {
    return (
      <div className="yearStatsContainer">
        <Radio.Group
          className="yearStatsTotalOrPercent"
          value={this.state.showPercent}
          onChange={(e) => this.setShowPercent(e.target.value)}
        >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={this.state.currentGraphData}>
            ...
            <YAxis />
            <Line
              dataKey={this.state.showPercent ? 'totalPercent' : 'total'}
              activeDot={{
                onClick: (p: { payload: { week: string } }) =>
                  this.onFormClick(p),
              }}
            />
          </LineChart>
