...
> {
  ...
  componentDidUpdate = (
    prevProps: MemberAttendanceProps,
    prevState: MemberAttendanceState
  ) => {
  ...
  getDisplayName = (): string | null => {
    if (this.props.member) {
      const displayName =
        ...
        (this.props.member.preferredName
          ? ` "${this.props.member.preferredName}" `
          : ' ') +
  ...
  getAttendance = (noLoad?: boolean) => {
    ...
    getAllForms()
      ...
      .catch((err) => {
        notification.error({
          message: 'Error getting attendance: ' + err.toString(),
        })
      })
    ...
    return (
      <div className="attendanceFormPopoverContainer">
        <SignupForm
          ...
          onSignupChanged={() => this.getAttendance(true)}
        ></SignupForm>
      </div>
    )
  }
          ...
          return (
            <div key={record.formTitle} className="attendanceLine">
              ...
              <p className="attendanceLineSession">
                ...
                <Popover
                  placement="rightTop"
                  content={this.getSignupPopover(record.formId)}
                  trigger="click"
                >
                  <Tooltip title="Edit">
